...
\restrict tCt6ecFecQfzPf1yyhCbLYIThKQT98gojm8j4b8ZMBhDaVAJQ6DfDF9PFeCY2RH
...
ALTER TABLE "rh"."employee_ponto_gestores" OWNER TO "postgres";


COMMENT ON TABLE "rh"."employee_ponto_gestores" IS 'Usuários que podem acompanhar o ponto deste funcionário no portal do gestor (acompanhamento/ponto).';
...
CREATE OR REPLACE FUNCTION "rh"."adjust_negative_hours_for_medical_certificate"("p_certificate_id" "uuid") RETURNS "void"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public', 'rh'
    AS $$
DECLARE
...
ALTER FUNCTION "rh"."adjust_negative_hours_for_medical_certificate"("p_certificate_id" "uuid") OWNER TO "postgres";


COMMENT ON FUNCTION "rh"."adjust_negative_hours_for_medical_certificate"("p_certificate_id" "uuid") IS 'Ajusta as horas negativas dos registros de ponto quando um atestado médico é aprovado. 
...
COMMENT ON FUNCTION "rh"."aprovar_ferias"("p_vacation_id" "uuid", "p_aprovado_por" "uuid") IS 'Aprova férias e atualiza dias gozados. aprovado_por é public.users.id (mesmo id do auth quando usuário tem registro em users).';
...
BEGIN
  ...
  -- Normalizar horas trabalhadas (evitar NULL)
  v_horas_trabalhadas := COALESCE(v_horas_trabalhadas, 0);

  -- Regra ESPECÍFICA para feriado/domingo/dia de folga:
  -- Todas as horas trabalhadas no dia são consideradas 100% (pagamento),
  -- não apenas o excedente da escala.
  IF (v_is_feriado OR v_is_domingo OR v_is_dia_folga) THEN
    ...
    -- Em dias de feriado/domingo/folga, não há horas para banco nem negativas
    v_horas_extras_50 := 0;
    v_horas_para_banco := 0;
    v_horas_negativas := 0;

  ELSE
    -- Dias normais continuam usando a lógica de excedente
    ...
    ELSIF v_excedente < 0 THEN
      -- Trabalhou MENOS que o esperado: horas negativas só se tiver todas as marcações
...
COMMENT ON FUNCTION "rh"."calculate_overtime_by_scale"("p_time_record_id" "uuid") IS 'Calcula horas extras conforme tipo de escala e regras CLT.
   CORREÇÃO: Em feriados, domingos e dias de folga, todas as horas trabalhadas
   são lançadas como horas_extras_100 (pagamento), e não apenas o excedente
   da escala. Dias normais continuam usando o excedente em relação às horas_diarias.';
...
CREATE OR REPLACE FUNCTION "rh"."create_rest_day_and_deduct_hours"("p_employee_id" "uuid", "p_company_id" "uuid", "p_data_folga" "date", "p_horas_descontar" numeric DEFAULT NULL::numeric, "p_observacoes" "text" DEFAULT NULL::"text", "p_created_by" "uuid" DEFAULT NULL::"uuid") RETURNS "uuid"
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
DECLARE
  v_rest_day_id UUID;
...
ALTER FUNCTION "rh"."create_rest_day_and_deduct_hours"("p_employee_id" "uuid", "p_company_id" "uuid", "p_data_folga" "date", "p_horas_descontar" numeric, "p_observacoes" "text", "p_created_by" "uuid") OWNER TO "postgres";


COMMENT ON FUNCTION "rh"."create_rest_day_and_deduct_hours"("p_employee_id" "uuid", "p_company_id" "uuid", "p_data_folga" "date", "p_horas_descontar" numeric, "p_observacoes" "text", "p_created_by" "uuid") IS 'Cria um dia de folga adicional para um funcionário e desconta automaticamente as horas do banco de horas';
...
CREATE OR REPLACE FUNCTION "rh"."diagnose_overtime_7th_day"("p_employee_id" "uuid", "p_company_id" "uuid", "p_date" "date") RETURNS TABLE("etapa" "text", "valor" "text")
    LANGUAGE "plpgsql" STABLE
    AS $$
DECLARE
  v_tipo_escala VARCHAR(50);
...
ALTER FUNCTION "rh"."diagnose_overtime_7th_day"("p_employee_id" "uuid", "p_company_id" "uuid", "p_date" "date") OWNER TO "postgres";


COMMENT ON FUNCTION "rh"."diagnose_overtime_7th_day"("p_employee_id" "uuid", "p_company_id" "uuid", "p_date" "date") IS 'Diagnóstico da regra de 7º dia (hora extra 100%). Retorna etapas e valores para um (employee_id, company_id, data). Use: SELECT * FROM rh.diagnose_overtime_7th_day(''uuid'', ''uuid'', ''2026-01-11''::date);';
...
CREATE OR REPLACE FUNCTION "rh"."get_effective_natureza_dia"("p_employee_id" "uuid", "p_company_id" "uuid", "p_date" "date") RETURNS "text"
    LANGUAGE "plpgsql"
    AS $$
DECLARE
  v_nat_override text;
  v_nat_record   text;
BEGIN
  -- Override manual de natureza do dia
  SELECT natureza_dia
  INTO v_nat_override
  ...
  IF v_nat_override IS NOT NULL THEN
    RETURN v_nat_override;
  END IF;

  -- Natureza armazenada no time_records (se houver)
  SELECT tr.natureza_dia
  INTO v_nat_record
  FROM rh.time_records tr
...
ALTER FUNCTION "rh"."get_effective_natureza_dia"("p_employee_id" "uuid", "p_company_id" "uuid", "p_date" "date") OWNER TO "postgres";
...
COMMENT ON FUNCTION "rh"."get_employee_work_shift_type"("p_employee_id" "uuid", "p_company_id" "uuid", "p_date" "date") IS 'Retorna o tipo de escala do funcionário na data. Busca em employee_shifts; se não houver, usa employees.work_shift_id.';
...
CREATE OR REPLACE FUNCTION "rh"."is_effective_rest_day"("p_employee_id" "uuid", "p_company_id" "uuid", "p_date" "date") RETURNS boolean
    LANGUAGE "plpgsql"
    AS $$
DECLARE
  v_nat text;
BEGIN
  v_nat := rh.get_effective_natureza_dia(p_employee_id, p_company_id, p_date);

  -- Se natureza foi ajustada para DSR ou Folga, tratar como dia de descanso
  IF v_nat IN ('dsr', 'folga') THEN
    RETURN true;
  END IF;

  -- Caso contrário, usar o dia de folga da escala (DSR do ciclo)
  RETURN rh.is_rest_day(p_employee_id, p_company_id, p_date);
END;
$$;


ALTER FUNCTION "rh"."is_effective_rest_day"("p_employee_id" "uuid", "p_company_id" "uuid", "p_date" "date") OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "rh"."is_virtual_dsr"("p_employee_id" "uuid", "p_company_id" "uuid", "p_date" "date") RETURNS boolean
    LANGUAGE "plpgsql" STABLE
    AS $$
BEGIN
...
ALTER FUNCTION "rh"."is_virtual_dsr"("p_employee_id" "uuid", "p_company_id" "uuid", "p_date" "date") OWNER TO "postgres";


COMMENT ON FUNCTION "rh"."is_virtual_dsr"("p_employee_id" "uuid", "p_company_id" "uuid", "p_date" "date") IS 'Retorna true se o dia for classificado como DSR pela natureza do dia (time_records.natureza_dia ou time_record_day_nature_override). Usado no cálculo de hora extra 100% em escalas flexíveis para quebrar o ciclo de 7 dias.';
...
DECLARE
  ...
  v_saida_extra1 TIME;
  v_horas_trabalhadas numeric(10,2) := 0;
...
COMMENT ON FUNCTION "rh"."recalculate_time_record_hours"("p_time_record_id" "uuid") IS 'Recalcula horas trabalhadas, faltas e noturnas a partir dos eventos. Chama calculate_overtime_by_scale.
CORREÇÃO 2026-02-10: Horas parciais quando há 2, 4 ou 6 marcações sem saída: usa último evento (ex.: entrada + início almoço = horas até início almoço).';
...
CREATE OR REPLACE FUNCTION "rh"."remove_rest_day_and_revert_deduction"("p_rest_day_id" "uuid") RETURNS boolean
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
DECLARE
  v_rest_day RECORD;
...
ALTER FUNCTION "rh"."remove_rest_day_and_revert_deduction"("p_rest_day_id" "uuid") OWNER TO "postgres";


COMMENT ON FUNCTION "rh"."remove_rest_day_and_revert_deduction"("p_rest_day_id" "uuid") IS 'Remove um dia de folga adicional e reverte o débito no banco de horas';
...
CREATE TABLE IF NOT EXISTS "rh"."time_records" (
    ...
    "natureza_dia" character varying(20) DEFAULT NULL::character varying,
    ...
    CONSTRAINT "time_records_natureza_dia_check" CHECK ((("natureza_dia" IS NULL) OR (("natureza_dia")::"text" = ANY ((ARRAY['normal'::character varying, 'dsr'::character varying, 'folga'::character varying, 'folga_debito'::character varying, 'feriado'::character varying, 'ferias'::character varying, 'atestado'::character varying, 'compensacao'::character varying, 'falta'::character varying, 'outros'::character varying])::"text"[])))),
...
COMMENT ON COLUMN "rh"."time_records"."natureza_dia" IS 'Natureza do dia: normal, DSR, folga, folga_debito (folga com débito de horas), feriado, férias, atestado, compensação, falta, outros. NULL = detecção automática.';
...
CREATE TABLE IF NOT EXISTS "rh"."medical_certificates" (
    ...
    CONSTRAINT "medical_certificates_tipo_atestado_check" CHECK ((("tipo_atestado")::"text" = ANY ((ARRAY['medico'::character varying, 'odontologico'::character varying, 'psicologico'::character varying, 'obito'::character varying, 'forcas_militares'::character varying, 'poder_judiciario'::character varying, 'servico_publico'::character varying, 'outros'::character varying])::"text"[])))
);
...
COMMENT ON TABLE "rh"."medical_certificates" IS 'Tabela de atestados médicos dos funcionários. IMPORTANTE: Atestados aprovados não impactam o banco de horas. Os dias de ausência não geram horas negativas ou débitos no banco de horas.';
...
ALTER TABLE "rh"."rest_day_schedule" OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "rh"."time_record_day_nature_override" (
    ...
    "company_id" "uuid" NOT NULL,
    "data_registro" "date" NOT NULL,
    "natureza_dia" character varying(20) NOT NULL,
    "created_at" timestamp with time zone DEFAULT "now"(),
    "updated_at" timestamp with time zone DEFAULT "now"(),
    CONSTRAINT "time_record_day_nature_override_natureza_check" CHECK ((("natureza_dia")::"text" = ANY ((ARRAY['normal'::character varying, 'dsr'::character varying, 'folga'::character varying, 'folga_debito'::character varying, 'feriado'::character varying, 'ferias'::character varying, 'atestado'::character varying, 'compensacao'::character varying, 'falta'::character varying, 'outros'::character varying])::"text"[])))
);


ALTER TABLE "rh"."time_record_day_nature_override" OWNER TO "postgres";


COMMENT ON TABLE "rh"."time_record_day_nature_override" IS 'Override manual da natureza do dia para datas que não têm registro em time_records (ex.: DSR, Férias, Atestado).';



COMMENT ON COLUMN "rh"."time_record_day_nature_override"."natureza_dia" IS 'Natureza do dia: normal, dsr, folga, folga_debito, feriado, ferias, atestado, compensacao, falta, outros.';
...
ALTER TABLE ONLY "rh"."employee_ponto_gestores"
    ADD CONSTRAINT "employee_ponto_gestores_employee_id_user_id_key" UNIQUE ("employee_id", "user_id");



ALTER TABLE ONLY "rh"."employee_ponto_gestores"
    ADD CONSTRAINT "employee_ponto_gestores_pkey" PRIMARY KEY ("id");
...
ALTER TABLE ONLY "rh"."rest_day_schedule"
    ADD CONSTRAINT "rest_day_schedule_employee_id_company_id_data_folga_key" UNIQUE ("employee_id", "company_id", "data_folga");



ALTER TABLE ONLY "rh"."rest_day_schedule"
    ADD CONSTRAINT "rest_day_schedule_pkey" PRIMARY KEY ("id");
...
ALTER TABLE ONLY "rh"."time_record_day_nature_override"
    ADD CONSTRAINT "time_record_day_nature_override_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "rh"."time_record_day_nature_override"
    ADD CONSTRAINT "time_record_day_nature_override_uniq" UNIQUE ("employee_id", "company_id", "data_registro");
...
CREATE UNIQUE INDEX "employees_company_id_cpf_active_unique" ON "rh"."employees" USING "btree" ("company_id", "cpf") WHERE (("status")::"text" <> 'demitido'::"text");



COMMENT ON INDEX "rh"."employees_company_id_cpf_active_unique" IS 'Permite que o mesmo CPF seja cadastrado na mesma empresa apenas se o funcionário anterior estiver demitido. Impede CPF duplicado para funcionários ativos/inativos/afastados/etc.';
...
CREATE INDEX "idx_employee_ponto_gestores_company_id" ON "rh"."employee_ponto_gestores" USING "btree" ("company_id");



CREATE INDEX "idx_employee_ponto_gestores_employee_id" ON "rh"."employee_ponto_gestores" USING "btree" ("employee_id");



CREATE INDEX "idx_employee_ponto_gestores_user_id" ON "rh"."employee_ponto_gestores" USING "btree" ("user_id");
...
CREATE INDEX "idx_rest_day_schedule_company" ON "rh"."rest_day_schedule" USING "btree" ("company_id");



CREATE INDEX "idx_rest_day_schedule_date" ON "rh"."rest_day_schedule" USING "btree" ("data_folga");



CREATE INDEX "idx_rest_day_schedule_employee" ON "rh"."rest_day_schedule" USING "btree" ("employee_id");
...
CREATE INDEX "idx_time_record_day_nature_override_employee_date" ON "rh"."time_record_day_nature_override" USING "btree" ("employee_id", "company_id", "data_registro");
...
CREATE OR REPLACE TRIGGER "update_rest_day_schedule_updated_at" BEFORE UPDATE ON "rh"."rest_day_schedule" FOR EACH ROW EXECUTE FUNCTION "public"."update_updated_at_column"();
...
ALTER TABLE ONLY "rh"."employee_ponto_gestores"
    ADD CONSTRAINT "employee_ponto_gestores_employee_id_fkey" FOREIGN KEY ("employee_id") REFERENCES "rh"."employees"("id") ON DELETE CASCADE;
...
ALTER TABLE ONLY "rh"."rest_day_schedule"
    ADD CONSTRAINT "rest_day_schedule_company_id_fkey" FOREIGN KEY ("company_id") REFERENCES "public"."companies"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "rh"."rest_day_schedule"
    ADD CONSTRAINT "rest_day_schedule_created_by_fkey" FOREIGN KEY ("created_by") REFERENCES "public"."profiles"("id");



ALTER TABLE ONLY "rh"."rest_day_schedule"
    ADD CONSTRAINT "rest_day_schedule_employee_id_fkey" FOREIGN KEY ("employee_id") REFERENCES "rh"."employees"("id") ON DELETE CASCADE;
...
ALTER TABLE ONLY "rh"."time_record_day_nature_override"
    ADD CONSTRAINT "time_record_day_nature_override_company_id_fkey" FOREIGN KEY ("company_id") REFERENCES "public"."companies"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "rh"."time_record_day_nature_override"
    ADD CONSTRAINT "time_record_day_nature_override_employee_id_fkey" FOREIGN KEY ("employee_id") REFERENCES "rh"."employees"("id") ON DELETE CASCADE;
...
ALTER TABLE ONLY "rh"."vacations"
    ADD CONSTRAINT "vacations_aprovado_por_fkey" FOREIGN KEY ("aprovado_por") REFERENCES "public"."users"("id") ON DELETE SET NULL;
...
CREATE POLICY "Users can delete time_record_day_nature_override from their com" ON "rh"."time_record_day_nature_override" FOR DELETE USING (("company_id" IN ( SELECT "user_companies"."company_id"
   FROM "public"."user_companies"
  WHERE ("user_companies"."user_id" = "auth"."uid"()))));
...
CREATE POLICY "Users can insert time_record_day_nature_override in their compa" ON "rh"."time_record_day_nature_override" FOR INSERT WITH CHECK (("company_id" IN ( SELECT "user_companies"."company_id"
   FROM "public"."user_companies"
  WHERE ("user_companies"."user_id" = "auth"."uid"()))));
...
CREATE POLICY "Users can manage rest day schedule for their companies" ON "rh"."rest_day_schedule" USING (("company_id" IN ( SELECT "user_companies"."company_id"
   FROM "public"."user_companies"
  WHERE ("user_companies"."user_id" = "auth"."uid"()))));
...
CREATE POLICY "Users can update time_record_day_nature_override from their com" ON "rh"."time_record_day_nature_override" FOR UPDATE USING (("company_id" IN ( SELECT "user_companies"."company_id"
   FROM "public"."user_companies"
  WHERE ("user_companies"."user_id" = "auth"."uid"()))));
...
CREATE POLICY "Users can view rest day schedule for their companies" ON "rh"."rest_day_schedule" FOR SELECT USING (("company_id" IN ( SELECT "user_companies"."company_id"
   FROM "public"."user_companies"
  WHERE ("user_companies"."user_id" = "auth"."uid"()))));
...
CREATE POLICY "Users can view time_record_day_nature_override from their compa" ON "rh"."time_record_day_nature_override" FOR SELECT USING (("company_id" IN ( SELECT "user_companies"."company_id"
   FROM "public"."user_companies"
  WHERE ("user_companies"."user_id" = "auth"."uid"()))));
...
CREATE POLICY "employee_ponto_gestores_delete" ON "rh"."employee_ponto_gestores" FOR DELETE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."user_companies" "uc"
  WHERE (("uc"."user_id" = "auth"."uid"()) AND ("uc"."company_id" = "employee_ponto_gestores"."company_id") AND ("uc"."ativo" = true)))));



CREATE POLICY "employee_ponto_gestores_insert" ON "rh"."employee_ponto_gestores" FOR INSERT TO "authenticated" WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."user_companies" "uc"
  WHERE (("uc"."user_id" = "auth"."uid"()) AND ("uc"."company_id" = "employee_ponto_gestores"."company_id") AND ("uc"."ativo" = true)))));



CREATE POLICY "employee_ponto_gestores_select" ON "rh"."employee_ponto_gestores" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."user_companies" "uc"
  WHERE (("uc"."user_id" = "auth"."uid"()) AND ("uc"."company_id" = "employee_ponto_gestores"."company_id") AND ("uc"."ativo" = true)))));



CREATE POLICY "employee_ponto_gestores_update" ON "rh"."employee_ponto_gestores" FOR UPDATE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."user_companies" "uc"
  WHERE (("uc"."user_id" = "auth"."uid"()) AND ("uc"."company_id" = "employee_ponto_gestores"."company_id") AND ("uc"."ativo" = true)))));
...
GRANT ALL ON TABLE "rh"."employee_ponto_gestores" TO "anon";
GRANT ALL ON TABLE "rh"."employee_ponto_gestores" TO "authenticated";
GRANT ALL ON TABLE "rh"."employee_ponto_gestores" TO "service_role";
...
GRANT ALL ON FUNCTION "rh"."adjust_negative_hours_for_medical_certificate"("p_certificate_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "rh"."adjust_negative_hours_for_medical_certificate"("p_certificate_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "rh"."adjust_negative_hours_for_medical_certificate"("p_certificate_id" "uuid") TO "service_role";
...
GRANT ALL ON FUNCTION "rh"."create_rest_day_and_deduct_hours"("p_employee_id" "uuid", "p_company_id" "uuid", "p_data_folga" "date", "p_horas_descontar" numeric, "p_observacoes" "text", "p_created_by" "uuid") TO "anon";
GRANT ALL ON FUNCTION "rh"."create_rest_day_and_deduct_hours"("p_employee_id" "uuid", "p_company_id" "uuid", "p_data_folga" "date", "p_horas_descontar" numeric, "p_observacoes" "text", "p_created_by" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "rh"."create_rest_day_and_deduct_hours"("p_employee_id" "uuid", "p_company_id" "uuid", "p_data_folga" "date", "p_horas_descontar" numeric, "p_observacoes" "text", "p_created_by" "uuid") TO "service_role";
...
GRANT ALL ON FUNCTION "rh"."diagnose_overtime_7th_day"("p_employee_id" "uuid", "p_company_id" "uuid", "p_date" "date") TO "anon";
GRANT ALL ON FUNCTION "rh"."diagnose_overtime_7th_day"("p_employee_id" "uuid", "p_company_id" "uuid", "p_date" "date") TO "authenticated";
GRANT ALL ON FUNCTION "rh"."diagnose_overtime_7th_day"("p_employee_id" "uuid", "p_company_id" "uuid", "p_date" "date") TO "service_role";
...
GRANT ALL ON FUNCTION "rh"."get_effective_natureza_dia"("p_employee_id" "uuid", "p_company_id" "uuid", "p_date" "date") TO "anon";
GRANT ALL ON FUNCTION "rh"."get_effective_natureza_dia"("p_employee_id" "uuid", "p_company_id" "uuid", "p_date" "date") TO "authenticated";
GRANT ALL ON FUNCTION "rh"."get_effective_natureza_dia"("p_employee_id" "uuid", "p_company_id" "uuid", "p_date" "date") TO "service_role";
...
GRANT ALL ON FUNCTION "rh"."is_effective_rest_day"("p_employee_id" "uuid", "p_company_id" "uuid", "p_date" "date") TO "anon";
GRANT ALL ON FUNCTION "rh"."is_effective_rest_day"("p_employee_id" "uuid", "p_company_id" "uuid", "p_date" "date") TO "authenticated";
GRANT ALL ON FUNCTION "rh"."is_effective_rest_day"("p_employee_id" "uuid", "p_company_id" "uuid", "p_date" "date") TO "service_role";
...
GRANT ALL ON FUNCTION "rh"."is_virtual_dsr"("p_employee_id" "uuid", "p_company_id" "uuid", "p_date" "date") TO "anon";
GRANT ALL ON FUNCTION "rh"."is_virtual_dsr"("p_employee_id" "uuid", "p_company_id" "uuid", "p_date" "date") TO "authenticated";
GRANT ALL ON FUNCTION "rh"."is_virtual_dsr"("p_employee_id" "uuid", "p_company_id" "uuid", "p_date" "date") TO "service_role";
...
GRANT ALL ON FUNCTION "rh"."remove_rest_day_and_revert_deduction"("p_rest_day_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "rh"."remove_rest_day_and_revert_deduction"("p_rest_day_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "rh"."remove_rest_day_and_revert_deduction"("p_rest_day_id" "uuid") TO "service_role";
...
GRANT ALL ON TABLE "rh"."rest_day_schedule" TO "anon";
GRANT ALL ON TABLE "rh"."rest_day_schedule" TO "authenticated";
GRANT ALL ON TABLE "rh"."rest_day_schedule" TO "service_role";
...
GRANT ALL ON TABLE "rh"."time_record_day_nature_override" TO "anon";
GRANT ALL ON TABLE "rh"."time_record_day_nature_override" TO "authenticated";
GRANT ALL ON TABLE "rh"."time_record_day_nature_override" TO "service_role";
...
\unrestrict tCt6ecFecQfzPf1yyhCbLYIThKQT98gojm8j4b8ZMBhDaVAJQ6DfDF9PFeCY2RH

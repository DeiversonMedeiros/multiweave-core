-- =====================================================
-- DEBUG DA FUNÇÃO get_entity_data PARA FERIADOS
-- Data: 2025-01-21
-- Descrição: Adiciona logs de debug para identificar problema com feriados
-- =====================================================

CREATE OR REPLACE FUNCTION public.get_entity_data_debug(
  schema_name text,
  table_name text,
...
BEGIN
    -- Log de entrada
    RAISE NOTICE 'get_entity_data_debug: INICIO - schema_name=%, table_name=%, company_id_param=%, filters=%', 
        schema_name, table_name, company_id_param, filters;
    
    -- Obter ID do usuario atual
    current_user_id := auth.uid();
    RAISE NOTICE 'get_entity_data_debug: current_user_id=%', current_user_id;
    
    -- Se nao ha usuario autenticado, negar acesso
    IF current_user_id IS NULL THEN
        RAISE NOTICE 'get_entity_data_debug: ERRO - Usuario nao autenticado';
    ...
    IF NOT public.check_access_permission(schema_name, table_name, 'read') THEN
        RAISE NOTICE 'get_entity_data_debug: ERRO - Acesso negado para %:%.%', schema_name, table_name, 'read';
    ...
    RAISE NOTICE 'get_entity_data_debug: user_companies=%', user_companies;
    ...
    IF public.is_admin_simple(current_user_id) THEN
        ...
        RAISE NOTICE 'get_entity_data_debug: Usuario e super admin';
    ELSE
        ...
        IF company_id_param IS NOT NULL THEN
            ...
            RAISE NOTICE 'get_entity_data_debug: Verificando acesso a empresa especifica: %', has_company_access;
        ELSE
            -- Se nao especificou empresa, permitir se tiver acesso a pelo menos uma
            has_company_access := (array_length(user_companies, 1) > 0);
            RAISE NOTICE 'get_entity_data_debug: Verificando acesso geral: %', has_company_access;
        END IF;
    END IF;
    
    -- Se nao tem acesso a empresa, negar
    IF NOT has_company_access THEN
        RAISE NOTICE 'get_entity_data_debug: ERRO - Acesso negado para empresa %', COALESCE(company_id_param, 'nao especificada');
        RAISE EXCEPTION 'Acesso negado para empresa %', COALESCE(company_id_param, 'nao especificada');
    END IF;
    
    -- Construir clausula WHERE basica
    where_clause := 'WHERE 1=1';
    RAISE NOTICE 'get_entity_data_debug: where_clause inicial=%', where_clause;
    ...
    IF company_id_param IS NOT NULL THEN
        ...
        BEGIN
            ...
            RAISE NOTICE 'get_entity_data_debug: company_uuid convertido=%', company_uuid;
        EXCEPTION
            WHEN OTHERS THEN
                RAISE NOTICE 'get_entity_data_debug: ERRO ao converter company_id_param para UUID: %', SQLERRM;
                RAISE EXCEPTION 'company_id_param invalido: %', company_id_param;
        END;
        
        where_clause := where_clause || ' AND company_id = $1';
        RAISE NOTICE 'get_entity_data_debug: where_clause com company_id=%', where_clause;
    ELSE
    ...
    IF filters IS NOT NULL AND jsonb_typeof(filters) = 'object' THEN
        ...
        LOOP
            RAISE NOTICE 'get_entity_data_debug: Processando filtro - key=%, value=%', filter_key, filter_value;
            ...
            IF filter_value IS NOT NULL AND filter_value != '' AND filter_value != 'all' THEN
                ...
                IF filter_key = 'ano' AND schema_name = 'rh' AND table_name = 'holidays' THEN
                    ...
                    RAISE NOTICE 'get_entity_data_debug: Aplicando filtro ano para holidays: %', condition;
                ...
                IF condition != '' THEN
                    ...
                    RAISE NOTICE 'get_entity_data_debug: Condicao adicionada: %', condition;
                END IF;
            END IF;
        END LOOP;
        ...
        IF array_length(filter_conditions, 1) > 0 THEN
            ...
            RAISE NOTICE 'get_entity_data_debug: where_clause final com filtros=%', where_clause;
        END IF;
    END IF;
    
    -- Construir clausula ORDER BY
    order_clause := 'ORDER BY ' || order_by || ' ' || order_direction;
    RAISE NOTICE 'get_entity_data_debug: order_clause=%', order_clause;
    
    -- Query para contar total de registros
    count_query := format('SELECT COUNT(*) FROM %I.%I %s', schema_name, table_name, where_clause);
    RAISE NOTICE 'get_entity_data_debug: count_query=%', count_query;
    
    -- Executar count query com parametros seguros
    IF company_id_param IS NOT NULL THEN
        EXECUTE count_query USING company_uuid INTO total_rows;
        RAISE NOTICE 'get_entity_data_debug: total_rows (com company_id)=%', total_rows;
    ELSE
        EXECUTE count_query USING user_companies INTO total_rows;
        RAISE NOTICE 'get_entity_data_debug: total_rows (sem company_id)=%', total_rows;
    END IF;
    ...
    RAISE NOTICE 'get_entity_data_debug: query_text=%', query_text;
    
    -- Executar query principal com parametros seguros
    IF company_id_param IS NOT NULL THEN
        RAISE NOTICE 'get_entity_data_debug: Executando query com company_uuid=%', company_uuid;
        RETURN QUERY EXECUTE query_text USING company_uuid;
    ELSE
        RAISE NOTICE 'get_entity_data_debug: Executando query com user_companies=%', user_companies;
        RETURN QUERY EXECUTE query_text USING user_companies;
    END IF;
    
    RAISE NOTICE 'get_entity_data_debug: FIM - Query executada com sucesso';
END;
$$;

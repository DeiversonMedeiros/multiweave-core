-- =====================================================
-- MIGRATION: FIX create_entity_data DATE TYPE HANDLING
-- =====================================================
-- Adiciona tratamento específico para o tipo DATE na função create_entity_data
  ...
  FOR key, value IN SELECT * FROM jsonb_each(data_param) LOOP
    ...
    ELSE
      ...
      ELSIF column_type = 'date' THEN
        -- DATE: converter string para DATE (ADICIONADO)
        values_list := values_list || quote_literal(value_str) || '::date';
        RAISE NOTICE '  → Tratado como DATE: %', quote_literal(value_str) || '::date';
...
'Função genérica para criar dados. Trata corretamente arrays, objetos JSONB, booleanos, DATE, TIMESTAMP e outros tipos. Verifica se a tabela tem company_id antes de inserir.';

-- =====================================================
-- CORREÇÃO: Função calculate_training_progress com debug
-- Problema: Retornando "0 de 0 aulas concluídas" mesmo com aulas concluídas
-- =====================================================
...
DECLARE
    ...
    v_total_content_inactive INTEGER;
    v_total_progress_records INTEGER;
    v_result JSONB;
BEGIN
    -- Verificar acesso
    IF NOT EXISTS (
    ...
    -- IMPORTANTE: Não filtrar por company_id aqui, pois training_content pode não ter company_id
    -- Se não encontrar conteúdos ativos, verificar se há algum conteúdo cadastrado
    SELECT COALESCE(COUNT(*), 0) INTO v_total_content
    FROM rh.training_content
    WHERE training_id = p_training_id
    AND is_active = true;
    
    -- Se não encontrou conteúdos ativos, verificar se há conteúdos inativos (para debug)
    IF v_total_content = 0 THEN
        SELECT COALESCE(COUNT(*), 0) INTO v_total_content_inactive
        FROM rh.training_content
        WHERE training_id = p_training_id;
        
        IF v_total_content_inactive > 0 THEN
            RAISE NOTICE '[calculate_training_progress] AVISO: Treinamento tem % conteúdos, mas todos estão inativos!', v_total_content_inactive;
        ELSE
            RAISE NOTICE '[calculate_training_progress] AVISO: Treinamento não tem conteúdos cadastrados!';
        END IF;
    END IF;

    -- Debug: Verificar se há conteúdos inativos também
    SELECT COALESCE(COUNT(*), 0) INTO v_total_content_inactive
    FROM rh.training_content
    WHERE training_id = p_training_id
    AND is_active = false;
    
    -- Debug: Log do total de conteúdos encontrados
    RAISE NOTICE '[calculate_training_progress] Total de conteúdos ativos encontrados: %', v_total_content;
    RAISE NOTICE '[calculate_training_progress] Total de conteúdos inativos: %', v_total_content_inactive;

    -- Contar conteúdos concluídos
    -- IMPORTANTE: Considerar múltiplas formas de verificar se está concluído:
    -- 1. concluido = true
    ...
    AND (
        tp.concluido = true 
        OR tp.percentual_concluido >= 100 
        OR tp.status = 'concluido'
    )
    AND tc.is_active = true
    AND tc.training_id = p_training_id;

    -- Debug: Verificar total de registros de progresso (mesmo não concluídos)
    SELECT COALESCE(COUNT(*), 0) INTO v_total_progress_records
    FROM rh.training_progress tp
    WHERE tp.training_id = p_training_id
    AND tp.employee_id = p_employee_id;
    
    RAISE NOTICE '[calculate_training_progress] Total de registros de progresso (todos): %', v_total_progress_records;
    
    -- Debug: Log do total de conteúdos concluídos encontrados
    RAISE NOTICE '[calculate_training_progress] Total de conteúdos concluídos: %', v_completed_content;
    RAISE NOTICE '[calculate_training_progress] Parâmetros: training_id=%, employee_id=%, company_id=%', 
        p_training_id, p_employee_id, p_company_id;
    ...
    RAISE NOTICE '[calculate_training_progress] Resultado final: %', v_result;

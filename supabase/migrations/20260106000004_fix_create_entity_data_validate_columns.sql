-- =====================================================
-- CORREÇÃO: create_entity_data - Validar colunas antes de inserir
-- Data: 2026-01-06
-- Descrição: Garante que apenas colunas que existem na tabela sejam inseridas
--            Isso resolve o erro "column usuario_id does not exist" ao criar contas a pagar
-- =====================================================
...
BEGIN
  ...
  FOR v_key, v_value IN SELECT * FROM jsonb_each(data_param) LOOP
    ...
    -- Esta é a correção crítica que resolve o erro de colunas inexistentes
    SELECT EXISTS (
      SELECT 1 
      FROM information_schema.columns col
      WHERE col.table_schema = v_schema_alias
    ...
    -- Se a coluna não existe, pular este campo (ex: usuario_id em contas_pagar)
    ...
    RAISE NOTICE '✅ ACEITANDO campo % (tipo coluna: %)', v_key, v_column_type;
  ...
  -- Verificar se há alguma coluna problemática (ex: usuario_id) e REMOVÊ-LA se existir
  -- Isso garante que mesmo que haja alguma inconsistência, não vamos tentar acessar colunas inexistentes
  IF 'usuario_id' = ANY(SELECT unnest(v_all_columns)) THEN
    RAISE WARNING '⚠️ ATENÇÃO: Coluna usuario_id encontrada na lista - REMOVENDO da lista de retorno';
    -- Remover usuario_id da lista
    SELECT array_agg(elem) 
    INTO v_all_columns
    FROM unnest(v_all_columns) AS elem
    WHERE elem != quote_ident('usuario_id');
    RAISE NOTICE '✅ Coluna usuario_id removida. Nova lista: %', array_to_string(v_all_columns, ', ');
  ELSE
    RAISE NOTICE '✅ Tabela %.% NÃO contém coluna usuario_id (correto)', v_schema_alias, v_table_alias;
  END IF;
...
Correção aplicada em 2026-01-06 para resolver erro ao criar contas a pagar.';

-- =====================================================
-- FIX: create_entity_data - Tratar strings vazias em campos DATE
-- =====================================================
-- Data: 2025-12-08
-- Descrição: Corrige o erro "invalid input syntax for type date: """
--            tratando strings vazias como NULL em campos de data
-- =====================================================
...
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public, rh, financeiro, core
AS $$
DECLARE
  result_json JSONB;
  ...
  FOR key, value IN SELECT * FROM jsonb_each(data_param) LOOP
    ...
    ELSE
      ...
      ELSIF column_type = 'date' THEN
        -- DATE: verificar se string está vazia antes de converter
        IF value_str IS NULL OR value_str = '' OR trim(value_str) = '' THEN
          values_list := values_list || 'NULL';
        ELSE
          values_list := values_list || quote_literal(value_str) || '::date';
        END IF;
      ELSIF column_type = 'time' OR column_type = 'time without time zone' THEN
        -- TIME: verificar se string está vazia
        IF value_str IS NULL OR value_str = '' OR trim(value_str) = '' THEN
          values_list := values_list || 'NULL';
        ELSE
          values_list := values_list || quote_literal(value_str);
      ...
      ELSIF column_type = 'timestamp' OR column_type = 'timestamp with time zone' OR column_type = 'timestamp without time zone' THEN
        -- TIMESTAMP: verificar se string está vazia
        IF value_str IS NULL OR value_str = '' OR trim(value_str) = '' THEN
          values_list := values_list || 'NULL';
        ELSE
          values_list := values_list || quote_literal(value_str);
        END IF;
      ELSE
        -- Para outros tipos de texto, se estiver vazio, usar NULL apenas se a coluna permitir
        IF value_str IS NULL OR value_str = '' THEN
...
-- Garantir permissões explícitas para PostgREST
...
'Função genérica para criar dados em qualquer schema. Trata corretamente arrays, objetos JSONB, booleanos, DATE, TIMESTAMP e outros tipos. Verifica se a tabela tem company_id antes de inserir. Trata strings vazias em campos de data como NULL. Exposta ao PostgREST via schema public.';

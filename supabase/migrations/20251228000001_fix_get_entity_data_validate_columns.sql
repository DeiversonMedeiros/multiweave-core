-- =====================================================
-- CORREÇÃO: get_entity_data - Validar colunas antes de usar
-- Data: 2025-12-28
-- Descrição: Valida se as colunas existem antes de usar em filtros e ordenação
--             Evita erros quando colunas não existem na tabela
-- =====================================================
...
BEGIN
    ...
    -- Se a coluna de ordenação não existe, usar 'created_at' ou 'id' como fallback
    IF NOT order_by_column_exists THEN
        RAISE NOTICE 'get_entity_data: Coluna de ordenação % não existe, usando fallback', order_by;
        ...
        IF order_by_column_exists THEN
            order_by := 'created_at';
        ELSE
            -- Se 'created_at' também não existe, usar 'id'
            order_by := 'id';
        END IF;
        RAISE NOTICE 'get_entity_data: Usando coluna de ordenação: %', order_by;
    END IF;
    ...
    ELSIF company_id_param IS NOT NULL AND NOT has_company_id_column THEN
        RAISE NOTICE 'get_entity_data: company_id_param fornecido mas tabela nao tem coluna company_id - ignorando filtro';
    END IF;
    ...
    IF filters IS NOT NULL AND jsonb_typeof(filters) = 'object' THEN
        ...
        LOOP
            ...
            IF filter_value IS NOT NULL AND filter_value != '' AND filter_value != 'all' THEN
                ...
                -- Determinar o nome da coluna base para validação
                ...
                -- Validar se a coluna existe antes de usar
                SELECT EXISTS (
                    SELECT 1 FROM information_schema.columns c
                    WHERE c.table_schema = schema_name
                    AND c.table_name = get_entity_data.table_name
                ...
                -- Se a coluna não existe, pular este filtro
                IF NOT column_exists THEN
                    RAISE NOTICE 'get_entity_data: Coluna % não existe na tabela %:%.% - ignorando filtro', 
                        base_field, schema_name, table_name;
                    CONTINUE;
                END IF;
...
-- Garantir permissões
GRANT EXECUTE ON FUNCTION public.get_entity_data(text, text, text, jsonb, integer, integer, text, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_entity_data(text, text, text, jsonb, integer, integer, text, text) TO anon;

-- Comentário
COMMENT ON FUNCTION public.get_entity_data(text, text, text, jsonb, integer, integer, text, text) IS 
'Função genérica para buscar dados. Valida se as colunas existem antes de usar em filtros e ordenação. Evita erros quando colunas não existem na tabela.';

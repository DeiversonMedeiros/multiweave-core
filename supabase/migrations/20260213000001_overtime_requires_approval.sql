-- =====================================================
-- APENAS REGISTROS COM HORA EXTRA EXIGEM APROVAÇÃO
-- =====================================================
-- Data: 2026-02-13
-- Objetivo: Dias com hora extra devem aparecer em
--           portal-gestor/aprovacoes/horas-extras como pendentes.
-- Alteração: Em recalculate_time_record_hours, após calculate_overtime_by_scale,
--            definir status = 'pendente' quando houver horas_extras_50/100/extras > 0;
--            caso contrário manter 'aprovado'. Rejeitados permanecem 'rejeitado'.
-- =====================================================
...
BEGIN
  ...
  -- Apenas registros com hora extra ficam pendentes (portal-gestor/aprovacoes/horas-extras)
  UPDATE rh.time_records
  SET status = CASE
    WHEN status = 'rejeitado' THEN 'rejeitado'
    WHEN COALESCE(horas_extras_50, 0) > 0 OR COALESCE(horas_extras_100, 0) > 0 OR COALESCE(horas_extras, 0) > 0 THEN 'pendente'
    ELSE 'aprovado'
  END
  WHERE id = p_time_record_id;
END;
...
'Recalcula horas trabalhadas, faltas e noturnas. Chama calculate_overtime_by_scale. Registros COM hora extra (50/100 ou horas_extras) ficam pendentes de aprovacao em portal-gestor/aprovacoes/horas-extras; demais sao aprovados automaticamente.';

-- Reabrir para aprovacao os registros ja aprovados que tem hora extra (passam a aparecer na pagina)
UPDATE rh.time_records
SET status = 'pendente',
    updated_at = COALESCE(updated_at, created_at, NOW())
WHERE status = 'aprovado'
  AND (COALESCE(horas_extras_50, 0) > 0 OR COALESCE(horas_extras_100, 0) > 0 OR COALESCE(horas_extras, 0) > 0);

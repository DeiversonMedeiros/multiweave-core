-- =====================================================
-- CORREÇÃO: Calcular horas trabalhadas para 2, 4 ou 6 marcações (horas parciais)
-- =====================================================
-- Data: 2026-02-10
-- Problema: Com apenas Entrada + Início almoço (2 marcações), o sistema não
--           calculava horas trabalhadas (Total de horas ficava --:--).
-- Causa: A migration 20260210000002 removeu o uso de "último evento" para
--        evitar saida da tabela; com isso deixou de existir o ramo que
--        calcula horas parciais quando não há saída (usando último evento).
-- Solução: Reintroduzir em recalculate_time_record_hours a lógica de horas
--          parciais: buscar último evento do dia e, quando não houver saída,
--          calcular horas até esse evento (ex.: último = entrada_almoco →
--          horas = entrada_almoco - entrada).
-- =====================================================

...
\restrict v6IIBwE5fO8VvTCYcvl3o8jx0d6fwurYxnC8pOJpXaOXBG77ypLkZlkdzNrnaMQ
...
COMMENT ON FUNCTION "rh"."calculate_and_accumulate_bank_hours"("p_employee_id" "uuid", "p_company_id" "uuid", "p_period_start" "date", "p_period_end" "date") IS 'Calcula e acumula horas extras 50% no banco de horas para um funcionário em um período.
...
CREATE OR REPLACE FUNCTION "rh"."calculate_night_hours"("p_entrada" time without time zone, "p_saida" time without time zone, "p_data_registro" "date") RETURNS numeric
...
BEGIN
  -- Se n?o tem entrada ou sa?da, retornar 0
  ...
  -- Se sa?da ? antes da entrada, assumir que ? no dia seguinte
  ...
  -- Per?odo noturno do dia atual (22h at? 23:59:59)
...
ALTER FUNCTION "rh"."calculate_night_hours"("p_entrada" time without time zone, "p_saida" time without time zone, "p_data_registro" "date") OWNER TO "postgres";


COMMENT ON FUNCTION "rh"."calculate_night_hours"("p_entrada" time without time zone, "p_saida" time without time zone, "p_data_registro" "date") IS 'Calcula horas trabalhadas no per?odo noturno (22h ?s 5h do dia seguinte). 
   Retorna 0 se n?o h? interse??o entre per?odo trabalhado e per?odo noturno.';
...
BEGIN
  ...
  -- VALIDA??O CR?TICA: Verificar se tem todas as marca??es principais
  ...
  -- CORRE??O CR?TICA: Buscar turno primeiro via employee_shifts (permite hist?rico)
  -- Se n?o encontrar, buscar via employees.work_shift_id (turno direto)
  -- IMPORTANTE: Garantir que v_horas_diarias nunca seja NULL se h? turno v?lido
  SELECT 
  ...
  -- CORRE??O: Se n?o encontrou via employee_shifts, buscar via employees.work_shift_id
  IF v_work_shift_id IS NULL OR v_horas_diarias IS NULL THEN
    SELECT 
      rh.get_employee_work_shift_type(v_employee_id, v_company_id, v_date),
      e.work_shift_id,
  ...
  -- Se encontrou turno, verificar se tem horarios_por_dia para o dia espec?fico
  IF v_work_shift_id IS NOT NULL THEN
    v_day_hours := rh.get_work_shift_hours_for_day(v_work_shift_id, v_day_of_week);
    
    -- Se tem hor?rio espec?fico para o dia, usar horas_diarias do JSONB
  ...
  -- CORRE??O CR?TICA: Se ainda n?o encontrou horas_diarias, tentar buscar do turno diretamente
  -- Isso garante que nunca use 8.0h como padr?o se h? um turno v?lido
  ...
  -- Se n?o encontrou turno, usar padr?o (?ltimo recurso)
  IF v_horas_diarias IS NULL THEN
    v_horas_diarias := 8.0;
    -- Log para debug (pode ser removido em produ??o)
    v_debug_info := format('AVISO: Usando padr?o 8.0h para funcion?rio %s em %s (turno n?o encontrado)', 
  ...
  -- Verificar se ? feriado, domingo ou dia de folga
  -- CORRE??O: Garantir que is_feriado seja atualizado corretamente
  ...
  -- =====================================================
  -- CORRE??O CR?TICA: L?GICA DE FERIADOS
  -- =====================================================
  -- Em feriados, TODAS as horas trabalhadas devem ser consideradas como extras 100%,
  -- n?o apenas o excedente. Isso inclui o turno normal + horas extras.
  -- =====================================================
  
  IF v_is_feriado AND v_horas_trabalhadas > 0 THEN
    -- FERIADO: Todas as horas trabalhadas s?o extras 100%
    -- N?o importa a escala, feriado sempre paga 100% de todas as horas
  ...
  ELSIF v_excedente > 0 THEN
    ...
    -- Aplicar regras por tipo de escala (apenas se N?O for feriado)
    IF v_tipo_escala = 'escala_12x36' THEN
      -- ESCALA 12x36: S? existe excedente se romper 12h
      IF v_horas_trabalhadas > 12 THEN
        v_excedente := v_horas_trabalhadas - 12;
        -- Horas extras normais v?o para banco (50%)
    ...
    ELSE
      ...
      IF v_is_domingo THEN
        ...
        v_horas_para_pagamento := v_excedente;
      ELSE
        -- S?bado em escala 5x2: vai para banco (50%)
        -- Horas extras normais: v?o para banco (50%)
        v_horas_extras_50 := v_excedente;
        v_horas_para_banco := v_excedente;
      END IF;
    END IF;
    -- Zerar horas negativas quando h? horas extras
    v_horas_negativas := 0;
    
  ELSIF v_excedente < 0 THEN
    -- Trabalhou MENOS que o esperado
    -- CORRE??O: S? calcular horas negativas se tem todas as marca??es
    -- Se n?o tem todas as marca??es, ? falta n?o registrada (n?o horas negativas)
    IF v_tem_todas_marcacoes THEN
      -- Tem todas as marca??es e trabalhou menos: calcular horas negativas
      v_horas_negativas := ROUND(ABS(v_excedente), 2);
    ELSE
      -- N?o tem todas as marca??es: n?o calcular horas negativas
      -- (ser? tratado como falta n?o registrada)
      v_horas_negativas := 0;
    END IF;
    -- Zerar horas extras quando h? horas negativas
    v_horas_extras_50 := 0;
  ...
  -- Atualizar registro
  -- horas_extras ? a soma de horas_extras_50 + horas_extras_100 (para compatibilidade)
...
COMMENT ON FUNCTION "rh"."calculate_overtime_by_scale"("p_time_record_id" "uuid") IS 'Calcula horas extras conforme tipo de escala e regras CLT.
   CORRE??O: Em feriados, TODAS as horas trabalhadas s?o consideradas como extras 100%,
   n?o apenas o excedente. Isso inclui o turno normal + horas extras.
   Todas as escalas consideram feriados para extras 100%.
   Garante que sempre use horas_diarias corretas do turno (nunca usa 8.0h como padr?o se h? turno v?lido).
   Separa horas 50% (banco) de horas 100% (pagamento direto).
   Considera horarios_por_dia para obter horas di?rias corretas por dia da semana.
   Calcula horas negativas quando trabalhou menos que o esperado E tem todas as marca??es.
   Calcula horas noturnas (22h ?s 5h).';
...
GRANT ALL ON FUNCTION "rh"."calculate_night_hours"("p_entrada" time without time zone, "p_saida" time without time zone, "p_data_registro" "date") TO "anon";
GRANT ALL ON FUNCTION "rh"."calculate_night_hours"("p_entrada" time without time zone, "p_saida" time without time zone, "p_data_registro" "date") TO "authenticated";
GRANT ALL ON FUNCTION "rh"."calculate_night_hours"("p_entrada" time without time zone, "p_saida" time without time zone, "p_data_registro" "date") TO "service_role";
...
\unrestrict v6IIBwE5fO8VvTCYcvl3o8jx0d6fwurYxnC8pOJpXaOXBG77ypLkZlkdzNrnaMQ

-- =====================================================
-- CORREÇÃO: get_entity_data - Tratar filtro "search" como busca em múltiplas colunas
-- Data: 2025-12-08
-- Descrição: Corrige a função get_entity_data para tratar o filtro "search" 
--            como uma busca em múltiplas colunas usando ILIKE, em vez de 
--            tentar usar "search" como uma coluna da tabela
-- =====================================================
        ...
        IF search_value IS NOT NULL AND search_value != '' AND search_value != 'all' THEN
            ...
            IF array_length(search_columns, 1) > 0 THEN
                ...
                FOR i IN 1..array_length(search_columns, 1) LOOP
                    ...
                    search_condition := search_condition || search_columns[i] || '::text ILIKE ''%' || 
    ...
    IF filters IS NOT NULL AND jsonb_typeof(filters) = 'object' THEN
        ...
        LOOP
            ...
            IF filter_value IS NOT NULL AND filter_value != '' AND filter_value != 'all' THEN
                ...
                ELSE
                    condition := filter_key || ' = ''' || replace(filter_value, '''', '''''') || '''';
                END IF;
...
Para rh.employees, busca em: nome, matricula, cpf, email.';

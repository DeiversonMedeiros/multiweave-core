-- =====================================================
-- Ajustar cálculo de horas extras 100% em feriados/domingo/folga
-- PRESERVANDO correções de overflow (numeric(4,2)) da versão 2026-02-05
-- =====================================================
-- Problema:
-- - Em feriados, domingos e dias de folga, apenas o excedente da escala
--   estava sendo considerado 100%, quando o esperado é que TODAS as horas
--   trabalhadas sejam 100% (pagamento).
--
-- Restrições técnicas:
-- - As colunas de horas em rh.time_records são NUMERIC(4,2), ou seja,
--   o valor absoluto deve ser < 100 para evitar erro 22003 (overflow).
--
-- Solução:
-- - Partir da função já corrigida em 2026-02-05 (que usa v_excedente
--   como NUMERIC(10,2) e limita os valores ao intervalo [-99.99, 99.99])
--   e apenas ajustar a regra de feriado/domingo/folga para:
--   * feriado/domingo/dia de folga: TODAS as horas trabalhadas vão para
--     horas_extras_100 / horas_para_pagamento (limitadas a 99.99)
--   * dias normais: manter a lógica de excedente da versão corrigida.
-- =====================================================
...
DECLARE
  ...
  v_excedente NUMERIC(10,2);  -- intermediário pode ser < -99.99 ou > 99.99
...
BEGIN
  ...
  -- Cálculo em NUMERIC(10,2) para não estourar; depois limitar a [-99.99, 99.99]
  v_excedente := v_horas_trabalhadas - v_horas_diarias;
  v_excedente := GREATEST(-99.99, LEAST(99.99, v_excedente));

  -- NOVA REGRA: Em feriado, domingo ou dia de folga,
  -- todas as horas trabalhadas vão para 100% (pagamento)
  IF (v_is_feriado OR v_is_domingo OR v_is_dia_folga) AND v_horas_trabalhadas > 0 THEN
    v_horas_extras_100 := LEAST(99.99, COALESCE(v_horas_trabalhadas, 0));
  ...
  ELSIF v_excedente > 0 THEN
    IF v_tipo_escala = 'escala_12x36' THEN
      IF v_horas_trabalhadas > 12 THEN
        v_excedente := LEAST(99.99, v_horas_trabalhadas - 12);
        v_horas_extras_50 := v_excedente;
        v_horas_para_banco := v_excedente;
      END IF;
    ELSIF v_tipo_escala = 'flexivel_6x1' THEN
      IF v_is_dia_folga THEN
        v_horas_extras_100 := LEAST(99.99, v_excedente);
        v_horas_para_pagamento := LEAST(99.99, v_excedente);
      ELSE
        v_horas_extras_50 := LEAST(99.99, v_excedente);
        v_horas_para_banco := LEAST(99.99, v_excedente);
      END IF;
    ELSE
  ...
  ELSIF v_excedente < 0 THEN
    IF v_tem_todas_marcacoes THEN
      -- Limitar a 99.99 para caber em DECIMAL(4,2)
...
CORREÇÃO 2026-02-05: v_excedente limitado a [-99.99, 99.99].
CORREÇÃO 2026-03-02: Em feriados, domingos e dias de folga, TODAS as horas trabalhadas vão para horas_extras_100/horas_para_pagamento (limitadas a 99.99).';

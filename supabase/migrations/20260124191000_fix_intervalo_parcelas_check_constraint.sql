-- =====================================================
-- FIX: Converter intervalo_parcelas de número para enum antes de inserir
-- Data: 2026-01-24
-- Descrição: O campo intervalo_parcelas em cotacao_fornecedores e pedidos_compra
--            armazena números (ex: '30', '60'), mas em contas_pagar há um CHECK
--            constraint que só aceita valores enum ('diario', 'semanal', 'mensal', etc.).
--            Esta migração corrige a função criar_conta_pagar para converter o valor.
-- Erro: SQLSTATE: 23514 (check constraint violation)
-- =====================================================
...
DECLARE
    ...
    v_intervalo_parcelas_raw VARCHAR(20); -- Valor original do pedido
    v_intervalo_parcelas_enum VARCHAR(20); -- Valor convertido para enum
    v_intervalo_dias INTEGER; -- Número de dias para cálculos
...
BEGIN
    ...
    -- ✅ FIX: Converter intervalo_parcelas de número para enum
    -- Primeiro, converter para número de dias
    BEGIN
        v_intervalo_dias := v_intervalo_parcelas_raw::INTEGER;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '[criar_conta_pagar] ⚠️ Erro ao converter intervalo_parcelas para número: %. Usando 30 dias como padrão.', v_intervalo_parcelas_raw;
            v_intervalo_dias := 30;
    END;
    
    -- Converter número de dias para enum válido
    ...
    RAISE NOTICE '[criar_conta_pagar] 💳 Condições de pagamento: forma_pagamento=%, is_parcelada=%, numero_parcelas=%, intervalo_raw=%, intervalo_dias=%, intervalo_enum=%', 
        v_forma_pagamento, v_is_parcelada, v_numero_parcelas, v_intervalo_parcelas_raw, v_intervalo_dias, v_intervalo_parcelas_enum;
    ...
    -- ✅ NOVO: Calcular data de vencimento baseada no intervalo de parcelas (usar número de dias)
    ...
    -- Usar intervalo_parcelas_enum (valor enum) ao invés de intervalo_parcelas_raw (número)
    ...
    INSERT INTO financeiro.contas_pagar (
        ...
        observacoes, 
        created_by,
        -- ✅ NOVOS CAMPOS: Condições de pagamento (usar enum)
        forma_pagamento,
        is_parcelada,
        numero_parcelas,
        intervalo_parcelas, -- ✅ FIX: Usar valor enum convertido
        pedido_id,
        numero_titulo
    ) VALUES (
        gen_random_uuid(), 
        ...
        v_is_parcelada,
        v_numero_parcelas,
        v_intervalo_parcelas_enum, -- ✅ FIX: Usar valor enum convertido
        p_pedido_id,
        v_numero_titulo
    ) RETURNING id INTO v_conta_id;
    
    RAISE NOTICE '[criar_conta_pagar] ✅ Conta criada com sucesso: conta_id=%, pedido_id=%, forma_pagamento=%, is_parcelada=%, numero_parcelas=%, intervalo_enum=%, numero_titulo=%, data_emissao=%', 
        v_conta_id, p_pedido_id, v_forma_pagamento, v_is_parcelada, v_numero_parcelas, v_intervalo_parcelas_enum, v_numero_titulo, v_data_emissao;
    ...
    IF v_is_parcelada AND v_numero_parcelas > 1 THEN
        ...
        FOR i IN 1..v_numero_parcelas LOOP
            -- Calcular data de vencimento da parcela (usar número de dias)
...
'Cria conta a pagar automaticamente quando pedido é criado. Inclui condições de pagamento, vinculação com pedido_id e criação automática de parcelas se is_parcelada = true. Corrigido em 2026-01-24 para converter intervalo_parcelas de número (dias) para enum válido antes de inserir na tabela contas_pagar.';

-- =====================================================
-- FIX: Garantir que all_photos e all_locations sempre retornem arrays JSONB válidos
-- e não NULL ou valores vazios incorretos
-- =====================================================
...
BEGIN
  ...
  SELECT
    ...
    ev.saida_latitude,
    ev.saida_longitude,
    -- FIX: Garantir que all_photos sempre seja um array JSONB válido (nunca NULL)
    COALESCE(ev.all_photos, '[]'::jsonb)::jsonb AS all_photos,
    -- FIX: Garantir que all_locations sempre seja um array JSONB válido (nunca NULL)
    COALESCE(ev.all_locations, '[]'::jsonb)::jsonb AS all_locations
  ...
  LEFT JOIN LATERAL (
    SELECT
      ...
      -- FIX: Usar COALESCE explícito e garantir que jsonb_agg nunca retorne NULL
      -- Se não houver fotos, retornar array vazio []
      -- Ordenar antes de agregar usando subquery ordenada
      COALESCE(
        (
          SELECT jsonb_agg(photo_obj)
          FROM (
      ...
      -- FIX: Usar COALESCE explícito e garantir que jsonb_agg nunca retorne NULL
      -- Se não houver localizações, retornar array vazio []
      -- Ordenar antes de agregar usando subquery ordenada
      COALESCE(
        (
          SELECT jsonb_agg(loc_obj)
          FROM (
            SELECT jsonb_build_object(
              'id', ee.id::text,
              'event_type', ee.event_type,

-- =====================================================
-- MELHORIA: Mensagem de erro mais informativa em process_approval
-- Data: 2026-01-25
-- Descrição: Melhora a mensagem de erro quando há aprovações pendentes
--            anteriores, incluindo informações sobre quem deve aprovar primeiro
-- =====================================================
...
BEGIN
    ...
    IF p_status = 'aprovado' THEN
        ...
        IF v_pending_count > 0 THEN
            -- Buscar detalhes das aprovações pendentes para incluir na mensagem
            v_pending_details := '';
            FOR v_pending_approval IN
                ...
                AND (
                    au.nivel_aprovacao < approval_record.nivel_aprovacao
                    OR (
                        au.nivel_aprovacao = approval_record.nivel_aprovacao
                        AND au.ordem < COALESCE(approval_record.ordem, 0)
                    )
                )
                ORDER BY au.nivel_aprovacao, au.ordem
                LIMIT 5
            LOOP
                IF v_pending_details != '' THEN
                    v_pending_details := v_pending_details || ', ';
                END IF;
                v_pending_details := v_pending_details || format(
                    'Nível %s, Ordem %s (%s)',
                    v_pending_approval.nivel_aprovacao,
                    v_pending_approval.ordem,
...
Melhorado em 2026-01-25: Mensagem de erro mais informativa, incluindo detalhes sobre quais aprovações estão pendentes e quem deve aprová-las.';

-- =====================================================
-- SCRIPT FINAL CORRIGIDO PARA COPIAR CONFIGURAÇÕES DO BANCO DE HORAS
-- O campo 'code' tem constraint UNIQUE global, então geramos códigos únicos
...
-- Gerando códigos únicos: se code original existe, adiciona sufixo "-TS", limitando a 20 caracteres
...
SELECT 
    ...
    bht.description,
    CASE 
        WHEN bht.code IS NOT NULL AND LENGTH(bht.code) > 0 THEN 
            -- Gera código único: código original (até 18 chars) + "-TS" = máximo 20 chars
            LEFT(bht.code, 18) || '-TS'
        ELSE NULL
    END as code,
...
-- Gerando códigos únicos: se code original existe, adiciona sufixo "-SV", limitando a 20 caracteres
...
SELECT 
    ...
    CASE 
        WHEN bht.code IS NOT NULL AND LENGTH(bht.code) > 0 THEN 
            -- Gera código único: código original (até 18 chars) + "-SV" = máximo 20 chars
            LEFT(bht.code, 18) || '-SV'
        ELSE NULL
    END as code,
    COALESCE(bht.has_bank_hours, true),
...
-- 6. Mostrar registros copiados com os novos códigos

-- =====================================================
-- CORREÇÃO DEFINITIVA: get_required_approvers - usar created_by em vez de usuario_id
-- Data: 2026-01-06
-- Descrição: Corrige definitivamente a função get_required_approvers para usar created_by
...
--            Esta migração garante que todas as versões anteriores sejam sobrescritas
-- =====================================================
...
BEGIN
    ...
    CASE p_processo_tipo
        WHEN 'conta_pagar' THEN
            -- CORREÇÃO CRÍTICA: usar created_by em vez de usuario_id
            -- A tabela contas_pagar NÃO possui coluna usuario_id
            SELECT 
                valor_original, 
                centro_custo_id, 
                projeto_id,
                departamento, 
                classe_financeira, 
                created_by  -- CORRIGIDO: era usuario_id
            INTO 
                processo_valor, 
                processo_centro_custo_id, 
                processo_projeto_id,
                processo_departamento, 
                processo_classe_financeira, 
                processo_usuario_id
        ...
        ELSE
            processo_valor := 0;
            processo_centro_custo_id := NULL;
            processo_projeto_id := NULL;
            processo_departamento := NULL;
            processo_classe_financeira := NULL;
            processo_usuario_id := NULL;
    END CASE;
...
'Corrigida definitivamente em 2026-01-06: usa created_by em vez de usuario_id para contas_pagar. 
A tabela contas_pagar não possui coluna usuario_id, apenas created_by. 
Esta versão garante que todas as versões anteriores sejam sobrescritas.';

-- =====================================================
-- FIX: Inserir itens de todas as requisições em cotação unificada
-- Data: 2026-02-17
-- Problema: Em cotação unificada (múltiplas requisições, mesmo numero_cotacao),
--           o cotacao_fornecedor pode ter itens vencedores de várias requisições.
--           O loop filtrava por ri.requisicao_id = v_cotacao_ciclo.requisicao_id,
--           o que fazia pedidos criados para fornecedores com itens de outras
--           requisições ficarem sem itens (ex: PED-000006).
-- Solução: Remover o filtro por requisicao_id - inserir TODOS os itens vencedores
--          do fornecedor na cotação, independente da requisição de origem.
-- =====================================================
    ...
    IF NEW.status = 'aprovada'
       ...
       AND (OLD.status IS NULL OR OLD.status != 'aprovada' OR OLD.workflow_state != 'aprovada') THEN
        ...
        LOOP
            ...
            -- ✅ FIX 2026-02-17: Remover filtro ri.requisicao_id = v_cotacao_ciclo.requisicao_id
            -- Em cotação unificada, o cotacao_fornecedor pode ter itens vencedores de várias requisições.
            -- Devemos inserir TODOS os itens que o fornecedor venceu, independente da requisição.
...
'Cria pedido e conta a pagar ao aprovar cotação. Usa fornecedores VENCEDORES. Em cotação unificada, insere itens de TODAS as requisições que o fornecedor venceu (fix 2026-02-17).';

-- =====================================================
-- CORREÇÃO: Tipo de parâmetro p_data_folga em call_schema_rpc
-- =====================================================
-- Data: 2026-02-11
-- Descrição: Ajusta a função genérica public.call_schema_rpc para
--            garantir que o parâmetro p_data_folga da função
--            rh.create_rest_day_and_deduct_hours seja tratado como DATE,
--            evitando erro de "função não existe" por incompatibilidade
--            de tipos (TEXT vs DATE).
-- =====================================================
...
BEGIN
    ...
    SELECT string_agg(
        CASE 
            ...
            -- Tratar p_data_folga como DATE especificamente para create_rest_day_and_deduct_hours
            WHEN key = 'p_data_folga' AND p_function_name = 'create_rest_day_and_deduct_hours' THEN
                CASE 
                    WHEN value::text = 'null' OR jsonb_typeof(value) = 'null' THEN 'NULL::DATE'
                    ELSE quote_literal(value #>> '{}') || '::DATE'
                END
...
- create_rest_day_and_deduct_hours: p_employee_id, p_company_id, p_data_folga (DATE), p_horas_descontar, p_observacoes, p_created_by
Parâmetros:

-- =====================================================
-- CORREÇÃO: Overflow na variável v_excedente em calculate_overtime_by_scale
-- =====================================================
-- Data: 2026-02-05
-- Erro: Ao aprovar correção de ponto (ex.: MARCIO ANJOS SILVA 26/01/2026),
--       RPC approve_attendance_correction retorna 400:
--       "numeric field overflow - A field with precision 4, scale 2 must round to an absolute value less than 10^2."
-- Causa: v_excedente é DECIMAL(4,2). recalculate_time_record_hours grava
--        horas_trabalhadas limitada a [-99.99, 99.99]. Em seguida
--        calculate_overtime_by_scale faz v_excedente := v_horas_trabalhadas - v_horas_diarias.
--        Ex.: -99.99 - 8 = -107.99 → estoura DECIMAL(4,2) na atribuição.
-- Solução: Usar NUMERIC(10,2) para v_excedente (cálculo intermediário) e
--          limitar ao intervalo [-99.99, 99.99] para gravação:
--          ex.: -8,00 permanece -8,00; -107,99 é limitado a -99,99.
-- =====================================================
...
CORREÇÃO 2026-02-05: v_excedente limitado a [-99.99, 99.99]: ex. -8,00 permanece -8,00; -107,99 grava como -99,99.';

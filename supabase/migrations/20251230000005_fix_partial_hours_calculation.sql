-- =====================================================
-- CORRIGIR CÁLCULO DE HORAS PARCIAIS
-- =====================================================
-- Problema: Registros com entrada e outros eventos mas sem saída
-- ficam com horas_trabalhadas = 0, mesmo quando o funcionário
-- registrou o ponto corretamente.
-- 
-- Solução: Calcular horas parciais usando o último evento do dia
-- como referência quando não houver saída explícita.
-- =====================================================

-- Atualizar função recalculate_time_record_hours para calcular horas parciais
...
-- Recalcular registros que têm eventos mas horas zeradas
DO $$
DECLARE
  v_record RECORD;
  v_count INTEGER := 0;
BEGIN
  RAISE NOTICE 'Iniciando recálculo de registros com eventos mas horas zeradas...';
  
  -- Recalcular registros que têm eventos mas horas_trabalhadas = 0 ou NULL
  FOR v_record IN

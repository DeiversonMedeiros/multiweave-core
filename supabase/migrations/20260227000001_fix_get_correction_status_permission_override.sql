-- =====================================================
-- CORREÇÃO: get_correction_status - permissão explícita deve sobrepor prazo
-- =====================================================
-- Data: 2026-02-27
-- Descrição: A verificação "v_permission_record IS NOT NULL" é pouco confiável para
--            tipo RECORD em PL/pgSQL. Usar FOUND após o SELECT para garantir que a
--            liberação feita no modal "Gerenciar Permissões de Correção" sobreponha
--            o prazo de liberação automática.
...
BEGIN
    ...
    -- Se tem permissão explícita liberada, retorna liberado (sobrepõe prazo)
...
COMMENT ON FUNCTION public.get_correction_status(UUID, INTEGER, INTEGER) IS 'Verifica se um funcionário pode fazer correções de ponto. Permissão explícita (modal Gerenciar Permissões) sobrepõe o prazo de liberação.';

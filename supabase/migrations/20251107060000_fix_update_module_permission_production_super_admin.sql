-- Corrigir função update_module_permission_production para aceitar Super Admin
-- O problema era que a função só verificava is_admin_all_production, que pode falhar
-- se o Super Admin não tiver permissões registradas em todos os módulos de produção.
-- Agora também verifica is_admin_simple que identifica diretamente o perfil "Super Admin"
...
-- Corrigir também a função update_entity_permission_production para consistência
CREATE OR REPLACE FUNCTION update_entity_permission_production(
  p_profile_id UUID,
  p_entity_name TEXT,
  p_action TEXT,
  p_value BOOLEAN

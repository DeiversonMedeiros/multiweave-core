-- Atualização: substitui critério de Departamento por Projeto nas configurações
-- de aprovação unificadas, mantendo compatibilidade com registros existentes.

-- 1) Estrutura: adicionar coluna de projeto e indexar
ALTER TABLE public.configuracoes_aprovacao_unificada
  ADD COLUMN IF NOT EXISTS projeto_id UUID REFERENCES public.projects(id);

CREATE INDEX IF NOT EXISTS idx_config_aprov_unif_projeto
  ON public.configuracoes_aprovacao_unificada(projeto_id);

-- Remover coluna legada de departamento (critério não será mais utilizado)
ALTER TABLE public.configuracoes_aprovacao_unificada
  DROP COLUMN IF EXISTS departamento;

-- 2) Função: recalcular aprovadores requeridos usando projeto_id
...
'Atualizada: usa projeto_id em vez de departamento; mantém suporte a valor_total_estimado para requisições de compra.';

...
import { Calendar, MapPin, Phone, Mail, User, Building, DollarSign, AlertCircle, UserCheck } from 'lucide-react';
...
const employeeSchema = z.object({
  ...
  tipo_cnh: z.enum(['A', 'B', 'C', 'D', 'E']).optional(),
...
export const EmployeeForm = forwardRef<EmployeeFormRef, EmployeeFormProps>(({ 
  ...
  const { availableUsers, isUserAlreadyLinked } = useEmployeeUser();
  ...
  const form = useForm<EmployeeFormData>({
    ...
    defaultValues: {
      ...
      certidao_casamento: employee?.certidao_casamento || '',
      ...
      tipo_cnh: employee?.tipo_cnh || undefined,
      ...
      <form id="employee-form" onSubmit={form.handleSubmit(handleSubmit)} className="space-y-8">
        <Tabs defaultValue="personal" className="w-full">
          <TabsList className="grid w-full grid-cols-7 mb-8 gap-1">
            <TabsTrigger value="personal" className="text-sm px-2 py-2">Dados Pessoais</TabsTrigger>
            <TabsTrigger value="documents" className="text-sm px-2 py-2">Documentos</TabsTrigger>
            <TabsTrigger value="banking" className="text-sm px-2 py-2">Dados Bancários</TabsTrigger>
            <TabsTrigger value="education" className="text-sm px-2 py-2">Escolaridade</TabsTrigger>
            <TabsTrigger value="professional" className="text-sm px-2 py-2">Profissionais</TabsTrigger>
            <TabsTrigger value="contact" className="text-sm px-2 py-2">Contato</TabsTrigger>
            <TabsTrigger value="address" className="text-sm px-2 py-2">Endereço</TabsTrigger>
          </TabsList>
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input 
                            {...field} 
                            placeholder="Número da certidão (se casado/divorciado)"
                            disabled={isReadOnly}
                          />
                        </FormControl>
                        <FormMessage />
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        >
                          ...
                          <SelectContent>
                            {employees
                              .filter(emp => emp.id !== employee?.id) // Não pode ser seu próprio gestor
                              .map((emp) => (
                              <SelectItem key={emp.id} value={emp.id}>
                                {emp.nome} - {emp.cargo?.nome || 'Sem cargo'}
                              </SelectItem>
                            ))}
                          </SelectContent>
                        </Select>

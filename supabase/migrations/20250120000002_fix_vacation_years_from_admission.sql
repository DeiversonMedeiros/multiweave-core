-- =====================================================
-- CORREÇÃO: ANOS DE FÉRIAS BASEADOS NA DATA DE ADMISSÃO
-- =====================================================

-- Função para calcular e criar períodos aquisitivos automaticamente baseados na data de admissão
...
-- NOTA: A função buscar_anos_ferias_disponiveis foi movida para o schema public
-- na migração 20250120000003_update_public_buscar_anos_ferias.sql
-- para simplificar a arquitetura e permitir acesso direto via REST API
-- Remover função antiga do schema rh se existir
DROP FUNCTION IF EXISTS rh.buscar_anos_ferias_disponiveis(UUID);

-- Atualizar constraint de status para incluir 'pendente' (períodos futuros)
ALTER TABLE rh.vacation_entitlements 
  DROP CONSTRAINT IF EXISTS vacation_entitlements_status_check;

ALTER TABLE rh.vacation_entitlements 
  ADD CONSTRAINT vacation_entitlements_status_check 
  CHECK (status IN ('ativo', 'vencido', 'gozado', 'parcialmente_gozado', 'pendente'));

-- Comentários
COMMENT ON FUNCTION rh.calcular_e_criar_periodos_aquisitivos IS 'Calcula e cria automaticamente períodos aquisitivos baseados na data de admissão do funcionário, incluindo períodos futuros para visualização';

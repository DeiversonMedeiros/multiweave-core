...
-- Adiciona validação para evitar conflito ao aprovar correção já aprovada
-- =====================================================
...
BEGIN
    ...
    IF v_correction IS NULL THEN
        RAISE EXCEPTION 'Correção não encontrada' USING ERRCODE = 'P0001';
    END IF;
    
    -- Verificar se a correção já foi aprovada
    IF v_correction.status = 'aprovado' THEN
        RAISE EXCEPTION 'Correção já foi aprovada anteriormente' USING ERRCODE = '23505';
    END IF;
    
    -- Verificar se a correção já foi rejeitada
    IF v_correction.status = 'rejeitado' THEN
        RAISE EXCEPTION 'Correção já foi rejeitada e não pode ser aprovada' USING ERRCODE = '23505';
    END IF;
    
    -- Verificar se a correção está pendente
    IF v_correction.status != 'pendente' THEN
        RAISE EXCEPTION 'Correção não está pendente (status: %)', v_correction.status USING ERRCODE = '23505';
    END IF;
    ...
    IF NOT FOUND THEN
        RAISE EXCEPTION 'Falha ao atualizar correção. Pode ter sido modificada por outro processo.' USING ERRCODE = '23505';
    END IF;
...
EXCEPTION
    WHEN unique_violation THEN
        RAISE EXCEPTION 'Conflito: Correção já foi processada por outro processo' USING ERRCODE = '23505';
    WHEN OTHERS THEN
        RAISE;
END;
...
'Aprova uma correção de ponto e atualiza o registro original incluindo almoço e hora extra.
 ...
 Inclui validações para evitar aprovar correções já processadas.';
